...
<article class="max-w-3xl mx-auto px-4 py-8 bg-white rounded shadow-md text-white" style="background: linear-gradient(to bottom, #827271 0%, #46445C 50%,  #827271 100%);">
    ...
    <form action="{{ route('user.catatan.update', [$catatanRenungan->id, $renungan->id]) }}" method="POST" class="bg-gray-600 p-4 rounded shadow-md">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="judul" class="form-label text-gray">Judul Renungan</label>
            <input type="text" class="form-control text-black border border-gray-300 rounded-lg p-2" id="judul" name="judul" value="{{ old('judul', $catatanRenungan->judul) }}" required style="border-width: 2px;">
        </div>

        <div class="mb-3">
            <label for="date_renungan" class="form-label text-gray">Tanggal Renungan</label>
            <input type="date" class="form-control text-black border border-gray-300 rounded-lg p-2" id="date_renungan" name="date_renungan" value="{{ old('date_renungan', $catatanRenungan->date_renungan) }}" required style="border-width: 2px;">
        </div>

        <div class="mb-3">
            <label for="prinsip" class="form-label text-gray">Prinsip</label>
            <textarea class="form-control text-black border border-gray-300 rounded-lg p-2" id="prinsip" name="prinsip" required rows="10" cols="50" style="resize: none; border-width: 2px;">{{ old('prinsip', $catatanRenungan->prinsip) }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control text-black border-gray-300 rounded-lg p-2" id="penerapan" name="penerapan" required>
                ...
                @foreach ([$renungan->penerapan1, $renungan->penerapan2, $renungan->penerapan3] as $penerapan)
                    <option value="{{ $penerapan }}" {{ old('penerapan', $catatanRenungan->penerapan) == $penerapan ? 'selected' : '' }}>
                        {{ $penerapan }}
                    </option>
                @endforeach
            </select>
